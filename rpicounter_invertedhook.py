...
def hook(self):
	...
	print(pin_status)
	if pin_status == 1: #reciever off the hook
	...
	if pin_status == 0: #receiver is hung up
		dial_list.clear()
...
if __name__ == '__main__':
	...
	hook('self')
	while True:
		if (pulse_count != 0) & ((time.time() - pulse_time) > 0.5) & (previous_pin_status == 1):
			print("Rotary Count")
			print(pulse_count)
			if pulse_count == 10:
				pulse_count = 0
